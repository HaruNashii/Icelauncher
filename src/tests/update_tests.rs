...
fn query_changed_filters_entries()
{
    let mut app = make_app_with_entries(
        vec![entry("Firefox"), entry("Vim"), entry("Firefox Dev")],
        1, 10,
    );
...
fn scrolled_to_top()
{
    let mut app = AppData { scroll_offset: 1.0, viewport_h: 300.0, content_h: 900.0, ..Default::default() };
