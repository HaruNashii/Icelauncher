...
pub fn view(app: &AppData) -> Element<'_, Message>
{
    ...
    let search_bar = container(search_input).padding(Padding { top: 0.0, right: 0.0, bottom: sc.bottom_margin, left: 0.0 });
    ...
    else
    {
        ...
        let items: Vec<Element<Message>> = app.filtered.iter().take(max).enumerate().map(|(i, entry)|
        {
            ...
            let row_content = row![icon_el, gap_el, label].align_y(Alignment::Center);

            let ep = ec.padding;
            button(row_content).on_press(Message::Launch(exec)).padding(Padding { top: ep[0] as f32, right: ep[1] as f32, bottom: ep[0] as f32, left: ep[1] as f32 }).width(Length::Fill).style(move |_theme, status| entry_button_style(status, is_selected, &cfg.clone())).into()
        }).collect();
        ...
        scrollable(column(grid_rows).spacing(wc.entry_spacing)).id(Id::new("results_scroll"))
            .on_scroll
            (|vp|
                Message::Scrolled
                (
    ...
    // ── Outer panel ──────────────────────────────────────────────────────────
    let content = column![search_bar, results, footer].spacing(wc.section_spacing).padding(wc.padding as f32);
